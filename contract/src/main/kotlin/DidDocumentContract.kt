...
class DidDocumentContract : ContractInterface {
    ...
    //创建DID文档
    @Transaction
    fun createDidDocument(ctx: Context,  didCreation: DidCreation) {
        val did = didCreation.did
        val exists = didDocumentExists(ctx, did)
        if (exists) {
            throw RuntimeException("The DID Document $did already exists")
        }
        val document= DidDocument(did= did, publicKey1 = didCreation.publicKey1, publicKey2 = didCreation.publicKey2, signatureValue = didCreation.signature)
    ...
    //查询DID文档
    @Transaction
    fun readDidDocument(ctx: Context, didRead: DidRead): DidDocument {
        val did = didRead.did
        val exists = didDocumentExists(ctx, did)
        if (!exists) {
    ...
    //更新DID文档
    @Transaction
    fun updateDidDocument(ctx: Context, didUpdate: DidUpdate) {
        val did = didUpdate.did
        val didRead=DidRead(did = did)
        val didDocument = readDidDocument(ctx, didRead)
    ...
    //删除DID文档
    @Transaction
    fun deleteDidDocument(ctx: Context, didRevoke: DidRevoke) {
        val did = didRevoke.did
        val didDocument = readDidDocument(ctx, DidRead(did = did))
